...
interface OrdersProps {
  onEditOrder?: (order: Gemstone) => void;
}
...
export default function Orders({ onEditOrder }: OrdersProps) {
  ...
  const filtered = gemstones
    ...
    .sort((a, b) => {
      // Sort by order number in descending order (newest first)
      const orderA = a.orderNumber || "";
      const orderB = b.orderNumber || "";

      // Extract numeric part from order number (e.g., "ORD-00001" -> 1)
      const numA = parseInt(orderA.split("-")[1] || "0", 10);
      const numB = parseInt(orderB.split("-")[1] || "0", 10);
  ...
  const handlePublishDraft = async (order: Gemstone) => {
    ...
    if ((order.frames?.length || 0) < MIN_FRAMES) {
      toast.error(
        `Draft needs at least ${MIN_FRAMES} frames before publishing`,
      );
      return;
    }
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {order.title && (
                      <p className="text-xs text-gray-warm truncate mt-0.5">
                        {order.title}
                      </p>
                    )}
                  </div>

                  <span className="text-sm text-charcoal font-mono truncate">
                  ...
                  <span className="text-sm text-charcoal text-center">
