...
export default function Orders() {
  ...
  const filtered = gemstones.filter((g) => {
    ...
    const matchesStatus =
      statusFilter === 'all' || (statusFilter === 'draft' ? g.status === 'draft' : g.status === 'completed');
  ...
  const handleDelete = async (order: Gemstone) => {
    if (confirm(`Delete order ${order.orderNumber || order.id} for ${order.customerName}? This cannot be undone.`)) {
      await deleteGemstone(order.id);
      toast.success('Order deleted');
    }
  };

  const handlePublishDraft = async (order: Gemstone) => {
    if (order.status !== 'draft') return;

    if (!order.title?.trim()) {
      toast.error('Draft needs a title before publishing');
      return;
    }

    if ((order.frames?.length || 0) < MIN_FRAMES) {
      toast.error(`Draft needs at least ${MIN_FRAMES} frames before publishing`);
      return;
    }
  ...
  return (
    <div className="space-y-8">
      <header className="pb-6 border-b border-gray-light/50 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          {(['all', 'draft', 'completed'] as StatusFilter[]).map((filter) => (
            <button
              key={filter}
              ...
              className={`h-9 px-4 rounded-lg text-sm font-medium transition-all ${
                statusFilter === filter
                  ? 'bg-gold text-white'
                  : 'bg-white border border-gray-light text-charcoal hover:bg-cream'
              }`}
            >
              {filter === 'all' ? 'All' : filter === 'draft' ? 'Drafts' : 'Completed'}
            </button>
          ))}
        </div>
      </header>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-warm text-sm">Try a different search or filter</p>
        </div>
      ) : (
        <div className="bg-white border border-gray-light/50 rounded-xl overflow-hidden">
          <div className="grid grid-cols-[110px_1fr_140px_90px_90px_110px_170px] gap-4 px-5 py-3 bg-cream/50 border-b border-gray-light/50 text-xs font-medium text-gray-warm uppercase tracking-wider">
            <span>Order #</span>
            <span>Customer</span>
            <span>Phone</span>
            <span>Tier</span>
          ...
          <div className="divide-y divide-gray-light/40">
            {filtered.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className="grid grid-cols-[110px_1fr_140px_90px_90px_110px_170px] gap-4 px-5 py-4 hover:bg-cream/20 transition-colors items-center"
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {order.title && <p className="text-xs text-gray-warm truncate mt-0.5">{order.title}</p>}
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    {isDraft && (
                      <button
                        onClick={() => handlePublishDraft(order)}
                        title="Publish draft"
                        className="h-8 px-3 rounded-lg border border-emerald/30 hover:bg-emerald/10 text-emerald text-xs font-semibold transition-all"
                      >
                        Publish
                      </button>
                    )}
                    ...
                    <button
                      ...
                      title={isDraft ? 'Upload frames / Edit draft' : 'Edit'}
                      className="w-8 h-8 rounded-lg border border-gray-light hover:bg-cream text-gray-warm hover:text-charcoal flex items-center justify-center transition-all"
                    >
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      {editingOrder && <EditOrder order={editingOrder} onClose={() => setEditingOrder(null)} />}
    </div>
  );
}
