...
export default function EditOrder({ order, onClose }: EditOrderProps) {
  ...
  const frameStatusLabel = getFrameValidationMessage(totalFrames, MIN_FRAMES, IDEAL_FRAMES);
  ...
  const uploadNewFrames = async (toastId: string): Promise<string[]> => {
    ...
    for (let i = 0; i < frameFiles.length; i += 1) {
      toast.loading(`Uploading frames... ${i + 1}/${frameFiles.length}`, { id: toastId });
      const url = await uploadToCloudinary(frameFiles[i], `orders/${order.id}/frames`);
      uploaded.push(url);
    }
    return uploaded;
  };

  const buildSharedUpdates = async (toastId: string): Promise<Partial<Gemstone>> => {
  ...
  const handleSave = async () => {
    ...
    if (!isDraft && tier === 'B' && !logoFile && !order.logoUrl) {
  ...
  const handlePublish = async () => {
    ...
    if (tier === 'B' && !logoFile && !order.logoUrl) {
      toast.error('Tier B requires a logo before publishing');
      return;
    }
    ...
    <div className="fixed inset-0 z-50 bg-charcoal/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-light/50">
          <div>
            <h2 className="font-serif text-xl text-charcoal">{isDraft ? 'Edit Draft' : 'Edit Order'}</h2>
        ...
        <div className="p-6 space-y-5 max-h-[72vh] overflow-y-auto">
          ...
          <div>
            <label className="block text-sm font-medium text-charcoal mb-2">Phone Number <span className="text-ruby">*</span></label>
            <input
              type="tel"
              value={phoneNumber}
          ...
          <div>
            <label className="block text-sm font-medium text-charcoal mb-2">Title {isDraft ? '' : <span className="text-ruby">*</span>}</label>
            <input
              type="text"
              value={title}
          ...
          <div>
            <label className="block text-sm font-medium text-charcoal mb-2">Frames</label>
            <div
              {...getFrameRootProps()}
              className={`p-4 border-2 border-dashed rounded-xl cursor-pointer transition-all ${
                isFrameDragActive ? 'border-gold bg-gold/5' : 'border-gray-light hover:border-gold/40'
              }`}
            >
              <input {...getFrameInputProps()} />
              <p className="text-sm text-charcoal text-center">Drop or click to add frames (PNG, JPG, WebP)</p>
              <p className="text-xs text-gray-warm text-center mt-1">Current: {order.frames?.length || 0}, New: {frameFiles.length}, Total: {totalFrames}</p>
              <p className="text-xs text-center mt-1 text-gray-warm">{frameStatusLabel}</p>
            </div>
            {frameFiles.length > 0 && (
              <button
          ...
          {tier === 'B' && (
            <div>
              <label className="block text-sm font-medium text-charcoal mb-2">
                Logo {!order.logoUrl && !logoFile && !isDraft && <span className="text-ruby">*</span>}
              </label>
              <div
                {...getLogoRootProps()}
                className={`p-5 border-2 border-dashed rounded-xl cursor-pointer transition-all ${
                  isLogoDragActive ? 'border-gold bg-gold/5' : 'border-gray-light hover:border-gold/40'
                }`}
              >
